...
**Android (`development-ai`):** `git fetch origin` and merge `origin/ios-development-ai` (recomendado). Último bloque compartido: `841ccef` (incluye fix de navegación tras export/share de proyecto + reportes `Name/ID` + tare por grupo + mejoras de alarmas). Si cherry-pick puntual: `841ccef` (y, si faltan, también `286dbc4` y `7ae7b33`). Rutas tocadas en este bloque reciente: `src/App.tsx`, `src/Layout/CommonLayout.tsx`, `src/context/AppContext.tsx`, `src/hooks/useFunctions.tsx`, `src/pages/Monitor/index.tsx`, `src/pages/Reports/index.tsx`, `src/services/alarmFeedback.ts`.
