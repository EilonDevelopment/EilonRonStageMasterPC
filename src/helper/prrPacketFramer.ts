 ...
 *
 * G4 weight: 5 bytes @ USB [5..9], checksum @ [10], footer `0E FF`.
 */
import {
  g4ChecksumValid,
  g4PacketStartsAtA5,
...
function isCrrWeightStreamHeader(frame: ArrayLike<number>, offset = 0): boolean {
  return frame[offset] === SYNC_FF
    && frame[offset + 1] === SYNC_A5
    && frame[offset + 2] === MSG_WEIGHT
    && frame[offset + 3] === CRR_STREAM_SUBTYPE;
}

function isA5CrrWeightStreamHeader(frame: ArrayLike<number>, offset = 0): boolean {
  return frame[offset] === SYNC_A5
    && frame[offset + 1] === MSG_WEIGHT
    && frame[offset + 2] === CRR_STREAM_SUBTYPE;
}

/** Valid CRR USB G4 weight frame: header + checksum + footer. */
export function isCrrG4UsbWeightFrame(frame: ArrayLike<number>): boolean {
  if (frame.length !== CRR_USB_G4_FRAME_LENGTH) return false;
  if (!isCrrWeightStreamHeader(frame, 0)) return false;
  const g4 = g4SliceFromUsbFrame(frame);
  if (!g4 || !g4PacketStartsAtA5(g4) || !g4ChecksumValid(g4)) return false;
  return hasG4UsbFooter(frame);
}
...
export function isFfA5V4At(buffer: ArrayLike<number>, offset = 0): boolean {
  return isCrrWeightStreamHeader(buffer, offset);
}

/** @deprecated alias */
export function isA5V4At(buffer: ArrayLike<number>, offset = 0): boolean {
  return isA5CrrWeightStreamHeader(buffer, offset);
}
...
export class CrrPacketFramer {
  ...
  private slideBufferToNextSync(): void {
    for (let i = 1; i < this.buffer.length - 3; i += 1) {
      if (isCrrWeightStreamHeader(this.buffer, i)) {
        this.buffer = this.buffer.slice(i);
        return;
      }
      if (isA5CrrWeightStreamHeader(this.buffer, i)) {
        this.buffer = this.buffer.slice(i);
        return;
      }
    }
  ...
  private tryExtractFrame(): Uint8Array | null {
    ...
    while (scanPos <= this.buffer.length - CRR_USB_FRAME_LENGTH) {
      if (isCrrWeightStreamHeader(this.buffer, scanPos)) {
      ...
      if (
        isA5CrrWeightStreamHeader(this.buffer, scanPos)
        && this.buffer.length - scanPos >= 13
      ) {
